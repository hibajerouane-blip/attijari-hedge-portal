...
interface Props {
  ...
  source: string;
  asOf: string;
  /** Si true, n'applique pas le style card (déjà dans un parent card). */
  bare?: boolean;
}
  ...
  return (
    <div
      className={
        bare
          ? ""
          : "rounded-xl border border-bank-200 bg-white p-5 shadow-card"
      }
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 font-mono text-3xl font-semibold tabular-nums text-bank-900">
            {formatFx(spot)}
          </div>
        </div>
        <span
